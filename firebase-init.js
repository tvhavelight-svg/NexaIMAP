...
async function loadAppStateFromFirestore() {
    try {
        ...
        const docRef = db.collection('satops').doc(APP_DOC_ID);
        const docSnap = await docRef.get().timeout(10000);
...
async function saveAppStateToFirestore() {
    try {
        ...
        await db.collection('satops').doc(APP_DOC_ID).set(state, { merge: true });
...
function startFirestoreListener() {
    unsubscribeFirestore = db.collection('satops').doc(APP_DOC_ID)
        .onSnapshot((doc) => {
            if (doc.exists) {
