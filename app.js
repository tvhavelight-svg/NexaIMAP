...
// Firestore save (async) - replaces localStorage saveAppState
function saveAppState() {
    if(suppressStateSave) return;
    saveAppStateToFirestore();
}

// Load from localStorage for initial setup (before Firestore loads)
...
function resetAllData() {
    ...
    // Clear Firestore
    db.collection('satops').doc('state').delete().then(() => {
        location.reload();
    });
}
